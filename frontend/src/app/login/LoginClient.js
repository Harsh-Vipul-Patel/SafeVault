...
export default function LoginClient() {
    ...
    // If user already has a valid token, redirect them away from login
    useEffect(() => {
        ...
        try {
            ...
            if (payload.exp && payload.exp < now) {
                // Token expired, let them log in fresh
                localStorage.clear();
                return;
            }
            const role = payload.role?.toUpperCase();
            if (role === 'CUSTOMER') router.replace('/customer/dashboard');
            else if (role === 'TELLER') router.replace('/teller/dashboard');
            else if (role === 'BRANCH_MANAGER') router.replace('/manager/dashboard');
            else if (role === 'LOAN_MANAGER') router.replace('/loan/dashboard');
            else if (role === 'SYSTEM_ADMIN') router.replace('/admin/dashboard');
        } catch {
